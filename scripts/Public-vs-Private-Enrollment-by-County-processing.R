...
##################################################################
#
# Processing Script for Public vs Private Enrollment by County
...
for (i in seq_along(yearlist)) {
  ...
  ## NUMBER MEASURES ##
    ...
    data <- melt(
            data,
            id.vars=c("NAME", "state", "Year", "Measure Type", "Variable"),
...
for (i in seq_along(yearlist)) {
  ...
  ## NUMBER MEASURES ##
    ...
    data <- melt(
            data,
            id.vars=c("NAME", "county", "Year", "Measure Type", "Variable"),
            variable.name="Grade;Type",
            variable.factor = F,
            value.name="Value",
            value.factor = F
...
dataset <- dataset %>% 
  select(NAME, county, Year, Grade, `School Type`, `Measure Type`, Variable, Value) %>% 
  rename(County = NAME, FIPS=county) %>% 
  arrange(County, Year, Grade, `School Type`, `Measure Type`, Variable)

write.table(
    dataset,
    file.path("data", "public-vs-private-enrollment-county-2016.csv"),
    sep = ",",
    row.names = F,
    na = "-9999"
)
